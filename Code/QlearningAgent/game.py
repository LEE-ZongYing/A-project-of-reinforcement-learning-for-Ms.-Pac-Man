...
class Game:
    ...
    def mute(self, agentIndex):
        ...
        import io
        OLD_STDOUT = sys.stdout #目前內部數值變成local variable 來裝 
        OLD_STDERR = sys.stderr #目前內部數值變成local variable 來裝
        sys.stdout = self.agentOutput[agentIndex] #將
    ...
    def run(self):
        ...
        while not self.gameOver:
            ...
            agent = self.agents[agentIndex]#0=pacman,1=ghost
            move_time = 0
            skip_action = False
            ...
            if self.catchExceptions:
                try:
                    ...
                    self.unmute()
                    return
            else:#will return the successor state after specified agent takes the
        ...
        for agentIndex, agent in enumerate(self.agents):
            if "final" in dir(agent):
                try:
                    ...
                    #QtableOutp=agent.final(self.state)#執行end的時候。
                    self.unmute()
                except Exception as data:
