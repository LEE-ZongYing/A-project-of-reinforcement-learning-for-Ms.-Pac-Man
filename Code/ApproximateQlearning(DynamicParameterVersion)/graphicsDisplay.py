...
class PacmanGraphics:
    ...
    def drawPacman(self, pacman, index):
        ...
        pacmanImageParts = []
        pacmanImageParts.append([circle(screen_point, PACMAN_SCALE * self.gridSize/1.5,fillColor =formatColor(1.0,1.0,1.0), outlineColor  =formatColor(1.0,1.0,1.0),endpoints = endpoints,width = width)])
        #pacmanImageParts.append([circle(screen_point, PACMAN_SCALE * self.gridSize,fillColor = fillColor, outlineColor = outlineColor,endpoints = endpoints,width = width)])
        

        return pacmanImageParts
    ...
    def drawGhost(self, ghost, agentIndex):
        ...
        #for (x, y) in GHOST_MOUSE:
            #coords2.append((x*self.gridSize*GHOST_SIZE + screen_x, y*self.gridSize*GHOST_SIZE + screen_y))
        colour = self.getGhostColor(ghost, agentIndex)
        body = polygon(coords, colour, filled = 1)
        #mouse = polygon(coords2, formatColor(1.0,1.0,1.0), filled = 1)
        #body = pygame.image.load("ghost.png").convert_alpha()
        ...
        if dir == 'West':
            dx = -0.2
        leftEye = circle((screen_x+self.gridSize*GHOST_SIZE*(-dx/1.5), screen_y-self.gridSize*GHOST_SIZE*(-dy/1.5)), self.gridSize*GHOST_SIZE*0.2, WHITE, WHITE)
        rightEye = circle((screen_x+self.gridSize*GHOST_SIZE*(dx/1.5), screen_y-self.gridSize*GHOST_SIZE*(-dy/1.5)), self.gridSize*GHOST_SIZE*0.2, WHITE, WHITE)
        leftPupil = circle((screen_x+self.gridSize*GHOST_SIZE*(-dx), screen_y-self.gridSize*GHOST_SIZE*(-dy)), self.gridSize*GHOST_SIZE*0.05, BLACK, BLACK)
        rightPupil = circle((screen_x+self.gridSize*GHOST_SIZE*(dx), screen_y-self.gridSize*GHOST_SIZE*(-dy)), self.gridSize*GHOST_SIZE*0.05, BLACK, BLACK)
        ghostImageParts = []
        ghostImageParts.append(body)
        #ghostImageParts.append(mouse)
    ...
    def moveEyes(self, pos, dir, eyes):
        ...
        if dir == 'West':
            dx = -0.2
        moveCircle(eyes[0],(screen_x+self.gridSize*GHOST_SIZE*(-0.3+dx/1.5), screen_y-self.gridSize*GHOST_SIZE*(-dy/1.5)), self.gridSize*GHOST_SIZE*0.2)
        moveCircle(eyes[1],(screen_x+self.gridSize*GHOST_SIZE*(0.3+dx/1.5), screen_y-self.gridSize*GHOST_SIZE*(-dy/1.5)), self.gridSize*GHOST_SIZE*0.2)
        moveCircle(eyes[2],(screen_x+self.gridSize*GHOST_SIZE*(-0.3+dx), screen_y-self.gridSize*GHOST_SIZE*(-dy)), self.gridSize*GHOST_SIZE*0.05)
        moveCircle(eyes[3],(screen_x+self.gridSize*GHOST_SIZE*(0.3+dx), screen_y-self.gridSize*GHOST_SIZE*(-dy)), self.gridSize*GHOST_SIZE*0.05)
